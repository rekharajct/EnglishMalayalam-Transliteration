...
parser.add_argument('-wp', '--wandb_project',  help='wandb project name', default="SeqToSeQ_att")
...
parser.add_argument("-rnn", "--rnn_type", help="Type of RNN choices: RNN, GRU, LSTM",  default="GRU")
parser.add_argument("-d", "--dropout", help="dropout probability", default="0.5",type=float)
parser.add_argument("-tf", "--teacher_forcing_ratio", help="teacher forcing ratio d ecoder", default="0.5",type=float)
...
parser.add_argument("-bi", "--bidirectional", help="True, False",  default="True")
parser.add_argument("-nl", "--num_layers", help="Number of layers in RNN",  default="3",type=int)
...
def transliterate():
  ...
  if config["rnn_type"]=="LSTM": #LSTM model
    #create model
    model, optimizer, criterion = create_model_LSTM(inp_dim,out_dim, config["enc_embed_dim"],\
          config["dec_embed_dim"], config["hidden_dim"], config["num_layers"], config["dropout_prob"],\
           config["bidirectional"],source, target, config["lr"], config["optimizer"])
  
  if config["rnn_type"]=="GRU": #GRU
    #create model
    model, optimizer, criterion = create_model_GRU(inp_dim,out_dim, config["enc_embed_dim"],\
          config["dec_embed_dim"], config["hidden_dim"], config["num_layers"], config["dropout_prob"],\
           config["bidirectional"],source, target, config["lr"],config["optimizer"])
    
  if config["rnn_type"]=="RNN": #RNN
    #create RNN model
    model, optimizer, criterion = create_model_RNN(inp_dim,out_dim, config["enc_embed_dim"],\
          config["dec_embed_dim"], config["hidden_dim"], config["num_layers"], config["dropout_prob"],\
           config["bidirectional"],source, target, config["lr"],config["optimizer"])
